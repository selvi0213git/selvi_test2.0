	...
	public function login_timeout(){
		if(is_user_logged_in()){
			...
			if($use_login_timeout){
				$login_timeout_url = add_query_arg(array('action'=>'cosmosfarm_members_login_timeout'), home_url());
				$login_timeout = $option->login_timeout * 60 * 1000;
				?>
				<script>
				window.onload = function(){
					setTimeout(function(){
						window.location.href = '<?php echo esc_url_raw($login_timeout_url)?>';
					}, <?php echo intval($login_timeout)?>);
				}
				</script>
				<?php
			}
