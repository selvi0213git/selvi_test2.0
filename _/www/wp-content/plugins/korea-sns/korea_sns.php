...
function kon_tergos_init() {
	...
	wp_enqueue_script('koreasns_js', plugins_url( 'korea_sns_156.js', __FILE__ ));
  ...
  for($i=0; $i<sizeof($arMobileAgent); $i++){ 
	...
	foreach($option['active_buttons'] as $snsKey => $snsOpt ){
		...
		if( $option['mobile_only'] && !$bMobileClient &&
				($snsKey=='kakaotalk' || $snsKey=='naverline')) continue;
		...
		switch( $snsKey )
		{
			...
			case 'naverband':
				if( $bMobileClient )
					...
					$call = "SendSNS('".$snsKey."-pc', '".$title." - ".$siteTitle."', '".$link."', '');";
						...
						<div class="inside">
							<table width="100%">
								...
								<tr>
									<td>&nbsp;</td>
									<td>
										<input type="checkbox" name="kon_tergos_mobile_only" <?php echo $check_mobile_only; ?>/> Hide mobile-click on the desktop (Kakaotalk, Naver Line)
									</td>
								</tr>
								<tr>
